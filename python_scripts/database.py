...
class Database(object):
  ...
  def find_main_pages(self):
    query = 'SELECT id FROM pages WHERE is_redirect=0'
    self.sdow_cursor.execute(query)
    query_result = self.sdow_cursor.fetchall()
    pages = [x[0] for x in query_result]
    self.main_pages = pages

  def find_leaves(self):
    query = 'SELECT id FROM links WHERE incoming_links_count=1 AND outgoing_links_count=1'
    self.sdow_cursor.execute(query)
    query_result = self.sdow_cursor.fetchall()
    pages = [x[0] for x in query_result]
    self.leaves = pages

  def get_random_main_page(self, leaf=False):
    if leaf:
      return random.choice(self.leaves)
    else:
      return random.choice(self.main_pages)


  def depth_first_search(self, start_id):
    try:
      add_column_query = "ALTER table links ADD visited BOOL"
      self.sdow_cursor.execute(add_column_query)
      self.sdow_conn.commit()
    except sqlite3.OperationalError:
      pass

    reset_column_query = "UPDATE links SET visited=0"
    self.sdow_cursor.execute(reset_column_query)
    self.sdow_conn.commit()


    update_visited_query = lambda id: f"UPDATE links SET visited=1 WHERE id={id}"
    self.sdow_cursor.execute(update_visited_query(start_id))
    self.sdow_conn.commit()
    ...
    for i in range(1):
      find_not_visited_query = 'SELECT id FROM pages WHERE visited=1'
      self.sdow_cursor.execute(find_not_visited_query)

      pages = [x[0] for x in self.sdow_cursor.fetchall()]
      for page in pages:
        print(page)

        get_outgoing_links_query = f'SELECT outgoing_links from links WHERE id={page}'
